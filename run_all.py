...
from score_downbeat import level_verdict, match
...
def main():
    ...
    p2_tot = {"tp": 0, "fp": 0, "fn": 0, "strict": 0, "acceptable": 0, "n": 0}
    ...
    for path in paths:
        ...
        level = level_verdict(p2["bar_ms"], seg["bar_ms"], tp, fp, fn)
        p2_tot["tp"] += tp
        p2_tot["fp"] += fp
        p2_tot["fn"] += fn
        p2_tot["strict"] += strict
        p2_tot["acceptable"] += (level != "wrong")
        p2_tot["n"] += 1
        ...
        report["segments"][seg["id"]] = {
            ...
            "phase2": {"tp": tp, "fp": fp, "fn": fn,
                       ...
                       "strict": strict, "level": level, "bar_ms": p2["bar_ms"],
        ...
        print(f"{seg['id']:<30} hands {p1_scores['prediction']['accuracy']*100:5.1f}%  "
              f"downbeats {recall*100:5.1f}% {level}")
    ...
    report["aggregate"] = {
        ...
        "phase2": {"recall": round(recall, 4), "precision": round(prec, 4),
                   "strict": p2_tot["strict"],
                   "acceptable": p2_tot["acceptable"], "n": p2_tot["n"]},
    }
    ...
    print(f"Phase 2: acceptable {p2_tot['acceptable']}/{p2_tot['n']} "
          f"(exact-level equivalence)  raw recall {recall*100:.1f}%  "
